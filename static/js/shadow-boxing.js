// https://github.com/bkardell/shadow-boxing
let globalStyles;
...
const filter =
  mode == "component-pull" || mode == "page-push" || mode == "page-push-select"
    ? ""
    : "[shadow-import]";
const isValidPushMode =
  mode == "page-push-marked" ||
  mode == "page-push" ||
  mode == "page-push-select" ||
  mode == "page-push-select-marked";
const isValidComponentPullMode =
  mode == "component-pull" || mode == "component-pull-marked";
...
function updateGlobalStyles() {
  globalStyles = document.head.querySelectorAll(
    `style${filter},link[rel="stylesheet"]${filter}`,
  );
}
      ...
      try {
        ...
        if (isValidComponentPullMode) {
          ...
          if (this.shadowRoot) {
            ...
            // if shadowRoot doesn't exist yet, wait to see if it gets added in connectedCallback
    ...
    disconnectedCallback() {
      try {
        ...
        if (isValidComponentPullMode) {
          ...
          if (delayedConnectedCallbackElements.has(this)) {
            // ensure our disconnected logic runs after our connected logic
            Promise.resolve().then(() => clearStyles(this));
          } else {
            // run immediately, no need to delay
            clearStyles(this);
          }
        }
      }
